...
| 項目 | サービス | 備考 |
|------|----------|------|
| **LP（ランディングページ）** | Wix | 広報委員でも操作しやすいと想定して採用 |
| **チケット申込・管理** | EventRegist | Premium機能を利用 |
| **オンデマンド配信** | Enavle | EventRegistと連携 |

### 現行構成の課題

- **Wix**: 広報委員が操作できるようにと導入したが、過去数年間、実質的に作業するのは事務局のみだった
- **EventRegist + Enavle**: 案1で詳述する課題あり（代理購入不可、プレーヤー品質、高コスト等）

> ⚠️ **代理申し込み不可**: 現行構成では各参加者が個別にチケットを購入する必要があり、法人が社員分を一括購入することができない。
...
| 項目 | 方針 | 理由 |
|------|------|------|
| **LP** | Vercel（必要に応じてCMS導入） | 事務局のみで運用するなら、より柔軟で低コストな構成が可能 |
| **チケット・配信** | 本資料で検討（案1〜3） | 現行の課題を踏まえて最適な構成を選定 |
...
**案1の課題ポイント**:
- **チケット購入は各参加者本人が個別に行う必要がある**（代理での一括購入不可）
- **法人の代理購入ユースケースに対応できない**
- プレーヤーの品質問題に対して事務局が改善を依頼しても反映されにくい
- 高額なシステム利用料（55万円〜/イベント）
...
> ⚠️ **案1の重要な制約**: チケット購入は**各参加者本人が個別にEventRegistで行う必要がある**（代理での一括購入不可）。Enavleはシンガポールのサービスであり、本人以外の個人情報を海外サーバーに移転できないため（個人情報保護）。EventRegist登録後はEnavleに自動連携される。
...
各案で使用する配信サービスの仕組みとコスト構造を比較する。

### サービス別の特徴

| サービス | 仕組み | 課金体系 | 特徴 |
|----------|--------|----------|------|
| **Enavle** | 外部動画サービスのプレーヤー埋め込み | ホスティングサービス費用＋Enavle利用料 | プレーヤーUI・再生品質はホスティング依存 |
| **OneStream** | 動画を直接アップロード | 月額固定（動画容量ベース） | 容量内なら視聴人数・時間に関係なく定額 |
| **Cloudflare Stream** | 動画を直接アップロード | 従量課金（保存＋視聴時間） | 安価だがDDoS対策必要 |
| **MUX** | 動画を直接アップロード | 従量課金（エンコード＋保存＋視聴） | 安価だがDDoS対策必要 |
...
- **現行**: YouTubeを使用
- **YouTubeの場合のみ**: Enavle側のプログラムで一部コントロール可能（ただし使いづらい）
- **課題**: ホスティングサービスが有料の場合、その費用も発生
...
- **課金**: 月額固定（プランごとに動画容量上限あり）
- **メリット**: 視聴人数・視聴時間に関係なく定額で予算が立てやすい
- **デメリット**: 容量を超えると上位プランへの変更が必要

### Cloudflare Stream / MUX（案3）の詳細
...
- **課金**: 従量課金（保存容量＋視聴分数）
- **メリット**: 使った分だけの支払いで、上記2つより安価
- **リスク**: WAF等で対策されていないとDDoS攻撃で料金が膨れ上がる可能性

> ⚠️ **セキュリティ対策**: Cloudflare StreamはCloudflareのWAF/DDoS対策と組み合わせることで、攻撃による従量課金リスクを軽減できる。

### コスト構造の比較

| 項目 | Enavle | OneStream | Cloudflare Stream |
|------|--------|-----------|-------------------|
| **初期費用** | Enavle利用料に含む | 0円 | 0円 |
| **月額基本料** | 高（Premium必須） | 9,800円〜 | なし |
| **動画保存** | ホスティング依存 | プラン内定額 | $5/1,000分/月 |
| **視聴課金** | ホスティング依存 | なし（定額） | $1/1,000分視聴 |
| **予算の立てやすさ** | △ | ◎ | △（従量制） |
| **スケーラビリティ** | ○ | △（容量上限） | ◎ |
...
| 代理申込（視聴登録） | - | EventRegist登録後、Enavleに自動連携 |
| 参加者情報収集 | ◎ | 独自アンケートで各参加者の情報を収集可能 |
| 招待チケット発行 | ◎ | プロモーションコード機能で無料/割引チケット発行 |
| 請求書払い | ◎ | Premium機能で対応（銀行振込も可） |
| 参加者別チェックイン | ◎ | QRコード付き宛名ラベル一括印刷対応 |

> ⚠️ **重要な制約**: チケット購入は各参加者本人が個別に行う必要がある（代理での一括購入不可）。EventRegist登録後はEnavleに自動連携されるため、Enavleへの再登録は不要。
>
> **代理購入不可の理由**: Enavleはシンガポールのサービスであり、本人以外の個人情報を海外サーバーに移転することは個人情報保護の観点から認められないため。
...
5. EventRegistからEnavleへアカウント自動連携
6. 視聴履歴は参加者単位で管理画面から確認可能

**この制約による影響**:
- 総務部が社員分を一括購入することができない
- 各参加者が個別にEventRegistで申込・決済を行う必要がある
- 法人の経費処理が煩雑になる（個人ごとの決済）
...
| **代理申込（視聴登録）** | - 自動連携 | ✕ 手動作業 | ◎ 自由設計 |
| **請求書払い対応** | ◎ 標準対応 | ✕ 非対応 | ○ 要実装 |
| **視聴権限の自動付与** | ◎ 自動連携 | ✕ 手動作業 | ◎ 設計次第 |
| **運用負荷** | △ 登録フォロー要 | △ 高い | △ 中程度 |
